...
namespace UniGLTF
{
    ...
    public class ImporterContext
    {
        ...
        public virtual void Parse(string path, Byte[] bytes)
        {
            ...
            {
                case ".gltf":
                    ParseJson(Encoding.UTF8.GetString(bytes), new FileSystemStorage(System.IO.Path.GetDirectoryName(path)));
                    break;
        ...
        public void CreateTextureItems(UnityPath imageBaseDir = default(UnityPath))
        {
            ...
            for (int i = 0; i < GLTF.textures.Count; ++i)
            {
                    ...
                    )
                {
                    ...
                    var textureName = !string.IsNullOrEmpty(image.name) ? image.name : System.IO.Path.GetFileNameWithoutExtension(image.uri);
